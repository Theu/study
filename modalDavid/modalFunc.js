    ...
    const checkEmailValue = () => {
        ...
        if (emailToTest.match(/^([a-z0-9_\.-]+)@([\da-z\.-]+)\.([a-z\.]{2,6})$/)) {
            ...
            emailInput.classList.remove('inputPassed')
    ...
    const showErrorPerson = () => {
        if ((firstNameInput.value === '') && (lastNameInput.value === '') && (emailInput.value === '')) {
            firstNameInput.classList.remove('inputPassed')
            lastNameInput.classList.remove('inputPassed')
            emailInput.classList.remove('inputPassed')
    ...
    const checkPhoneNumber = () => {
        ...
        if (((phoneToTest.match(/^\(?([0-9]{3,})\)?[- ]?([0-9]{3,})$/)) && (!phoneToTest.match(/^\(?([0-9]{3,})\)?[- ]?([0-9]{3,}?[- ])$/)) )|| (phoneToTest.match(/^\(?([0-9]{3,})\)?[- ]?([0-9]{3,})?[- ]?([0-9]{3,})$/))) {
                phoneInput.classList.add('inputPassed')
            ...
            phoneInput.classList.remove('inputPassed')
